...
!
    ...
    !
    ! Utitity procedures for obtaining the indices for a face
    !

    subroutine get_own_face_indices(idomain,ielement,iface)
        class(chidg_worker_t),  intent(in)  :: self
        integer(ik),            intent(inout) :: idomain, ielement, iface

        
        idomain = self%mesh%domain(self%element_info%idomain_l)%faces(self%element_info%ielement_l, self%iface)%ineighbor_domain_l
        ielement = self%mesh%domain(self%element_info%idomain_l)%faces(self%element_info%ielement_l, self%iface)%ineighbor_element_l
        iface = self%mesh%domain(self%element_info%idomain_l)%faces(self%element_info%ielement_l, self%iface)%ineighbor_face_l

    end subroutine get_own_face_indices

    
    subroutine get_neighbor_face_indices(idomain,ielement,iface)
        class(chidg_worker_t),  intent(in)  :: self
        integer(ik),            intent(inout) :: idomain, ielement, iface
    ...
    end subroutine get_neighbor_face_indices

    subroutine get_face_indices(idomain,ielement,iface,interp_source)
        class(chidg_worker_t),  intent(in)  :: self
        integer(ik),            intent(inout) :: idomain, ielement, iface
        character(*),           intent(in)  :: interp_source
        ...
        if ((interp_source == 'face interior') .or. (interp_source == 'boundary')) then
            call get_own_face_indices(idomain, ielement, iface)
        else if (interp_source == 'face_exterior') then
            call get_neighbor_face_indices(idomain, ielement, iface)
        else
            user_msg = "chidg_worker%get_face_indices: Invalid value for interpolation source. &
                        Try 'face interior', 'face exterior', or 'boundary'"
            call chidg_signal_one(FATAL,user_msg,trim(interp_source))
        end if

    end subroutine get_face_indices
    ...
    !!
    !----------------------------------------------------------------------------------------------------
    function get_grid_velocity_face(self,field,interp_source) result(grid_vel_gq)
        class(chidg_worker_t),  intent(in)  :: self
        character(len=*),       intent(in)  :: field
        character(*),           intent(in)  :: interp_source
        ...
        integer(ik)     :: idomain, ielement, iface

        call get_face_indices(idomain,ielement,iface, interp_source)

        if (field == 'u_grid') then
            grid_vel_gq = self%mesh%domain(idomain)%faces(ielement, iface)%grid_vel(:,1)

        else if (field == 'v_grid') then

            grid_vel_gq = self%mesh%domain(idomain)%faces(ielement, iface)%grid_vel(:,2)
        else if (field == 'w_grid') then

            grid_vel_gq = self%mesh%domain(idomain)%faces(ielement, iface)%grid_vel(:,3)
        else
    ...
    !!
    !----------------------------------------------------------------------------------------------------
    function get_jacobian_grid_face(self, interp_source) result(jacobian_grid_gq)
        class(chidg_worker_t),  intent(in)  :: self
        character(*),           intent(in)  :: interp_source
        ...
        jacobian_grid_gq = self%mesh%domain(idomain)%faces(ielement, iface)%jacobian_grid(:,:,:)
    ...
    function get_inv_jacobian_grid_face(self,interp_source) result(jacobian_grid_gq)
        ...
        jacobian_grid_gq = self%mesh%domain(idomain)%faces(ielement, iface)%inv_jacobian_grid(:,:,:)
    ...
    function get_det_jacobian_grid_face(self, interp_type,interp_source) result(det_jacobian_grid_gq)
        ...
        if (interp_type == 'value') then
            val = &
            self%mesh%domain(idomain)%faces(ielement,iface)%basis_s%interpolator('Value',iface)
        else if (interp_type == 'grad1') then
            val = self%mesh%domain(idomain)%faces(ielement,iface)%grad1
        else if (interp_type == 'grad2') then
            val = self%mesh%domain(idomain)%faces(ielement,iface)%grad2
        else if (interp_type == 'grad3') then
            val = self%mesh%domain(idomain)%faces(ielement,iface)%grad3
        end if


        det_jacobian_grid_gq = &
        matmul(val,self%mesh%domain(idomain)%elems(ielement)%det_jacobian_grid_modes)
