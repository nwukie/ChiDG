...
#include <messenger.h>
    ...
    !!
    !----------------------------------------------------------
    type, private :: mr4vector_wrapper_t

        real(rk),   allocatable :: mat(:,:,:,:)

    end type mr4vector_wrapper_t
    !**********************************************************
    ...
    !!
    !--------------------------------------------------------------------------------
    type, public :: mr4vector_t
        integer(ik)             :: size_        = 0
        integer(ik)             :: capacity_    = 0
        integer(ik)             :: buffer_      = 20

        type(mr4vector_wrapper_t), allocatable :: data(:)
...
contains
    ...
    function size(self) result(res)
        class(mr4vector_t),   intent(in)  :: self
    ...
    subroutine push_back(self,element)
        class(mr4vector_t),   intent(inout)   :: self
    ...
    subroutine increase_capacity(self)
        ...
        type(mr4vector_wrapper_t), allocatable   :: temp(:)
