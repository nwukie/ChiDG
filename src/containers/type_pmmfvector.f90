...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------
    type, public :: pmmfvector_t
        integer(ik)             :: size_        = 0
        integer(ik)             :: capacity_    = 0
        integer(ik)             :: buffer_      = 20

        type(prescribed_mesh_motion_function_wrapper_t), allocatable :: data(:)
...
contains
    ...
    function size(self) result(res)
        class(pmmfvector_t),   intent(in)  :: self
    ...
    subroutine push_back(self,element)
        class(pmmfvector_t),       intent(inout)   :: self
        class(prescribed_mesh_motion_function_t),        intent(in)      :: element
        ...
        !
        size = self%size()
        allocate(self%data(size + 1)%pmmf, source=element, stat=ierr)
    ...
    function at(self,index) result(res)
        ...
        integer                         :: ierr
        class(prescribed_mesh_motion_function_t),  allocatable :: res
        ...
        if (out_of_bounds) then
            call chidg_signal(FATAL,'pmmfvector_t%at: out of bounds access')
        end if
        ...
        ! Allocate result
        !
        allocate(res, source=self%data(index)%pmmf, stat=ierr)
        if (ierr /= 0) call chidg_signal(FATAL,"pmmfvector%at: error returning boundary condition")
    ...
    function index_by_name(self,key) result(ind)
        ...
        do ipmmf = 1,npmmfs
            ...
            !
            fname = self%data(ipmmf)%pmmf%get_name()
    ...
    subroutine increase_capacity(self)
        ...
        type(prescribed_mesh_motion_function_wrapper_t), allocatable   :: temp(:)
