...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------
    type, public :: pmmvector_t
        integer(ik)             :: size_        = 0
        integer(ik)             :: capacity_    = 0
        integer(ik)             :: buffer_      = 20

        type(prescribed_mesh_motion_wrapper_t), allocatable :: data(:)
...
contains
    ...
    function size(self) result(res)
        class(pmmvector_t),   intent(in)  :: self
    ...
    subroutine push_back(self,element)
        class(pmmvector_t),       intent(inout)   :: self
        class(prescribed_mesh_motion_t),        intent(in)      :: element
        ...
        !
        size = self%size()
        allocate(self%data(size + 1)%pmm, source=element, stat=ierr)
    ...
    function at(self,index) result(res)
        ...
        integer                         :: ierr
        class(prescribed_mesh_motion_t),  allocatable :: res
        ...
        if (out_of_bounds) then
            call chidg_signal(FATAL,'pmmvector_t%at: out of bounds access')
        end if
        ...
        ! Allocate result
        !
        allocate(res, source=self%data(index)%pmm, stat=ierr)
        if (ierr /= 0) call chidg_signal(FATAL,"pmmvector%at: error returning boundary condition")
    ...
    function index_by_name(self,key) result(ind)
        ...
        do ipmm = 1,npmms
            ...
            !
            fname = self%data(ipmm)%pmm%get_name()
    ...
    subroutine increase_capacity(self)
        ...
        type(prescribed_mesh_motion_wrapper_t), allocatable   :: temp(:)
