...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------
    type, public :: rbfvector_t
        integer(ik)             :: size_        = 0
        integer(ik)             :: capacity_    = 0
        integer(ik)             :: buffer_      = 20

        type(radial_basis_function_wrapper_t), allocatable :: data(:)
...
contains
    ...
    function size(self) result(res)
        class(rbfvector_t),   intent(in)  :: self
    ...
    subroutine push_back(self,element)
        class(rbfvector_t),       intent(inout)   :: self
        class(radial_basis_function_t),        intent(in)      :: element
        ...
        !
        size = self%size()
        allocate(self%data(size + 1)%rbf, source=element, stat=ierr)
    ...
    function at(self,index) result(res)
        ...
        integer                         :: ierr
        class(radial_basis_function_t),  allocatable :: res
        ...
        if (out_of_bounds) then
            call chidg_signal(FATAL,'rbfvector_t%at: out of bounds access')
        end if
        ...
        ! Allocate result
        !
        allocate(res, source=self%data(index)%rbf, stat=ierr)
        if (ierr /= 0) call chidg_signal(FATAL,"rbfvector%at: error returning boundary condition")
    ...
    function index_by_name(self,key) result(ind)
        ...
        do irbf = 1,nrbfs
            ...
            !
            fname = self%data(irbf)%rbf%get_name()
    ...
    subroutine increase_capacity(self)
        ...
        type(radial_basis_function_wrapper_t), allocatable   :: temp(:)
