module BU_boundary_average_advective_flux
#include <messenger.h>
    use mod_kinds,                  only: rk,ik
    use mod_constants,              only: NFACES,ZERO,ONE,TWO,HALF,FOUR, &
    ...
    type, extends(boundary_flux_t), public :: BU_boundary_average_advective_flux_t


    contains
        procedure   :: compute

    end type BU_boundary_average_advective_flux_t
...
contains
    ...
    subroutine compute(self,mesh,sdata,prop,face_info,function_info)
        class(BU_boundary_average_advective_flux_t),    intent(in)      :: self
        ...
        !
        flux_x = cx * ((u_l**TWO + u_r**TWO)/FOUR )  *  norms(:,1)
        flux_y = cy * ((u_l**TWO + u_r**TWO)/FOUR )  *  norms(:,2)
        flux_z = cz * ((u_l**TWO + u_r**TWO)/FOUR )  *  norms(:,3)
...
end module BU_boundary_average_advective_flux
