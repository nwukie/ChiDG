...
contains
    ...
    subroutine compute(self,mesh,sdata,ielem,iface,iblk,prop)
        ...
        !
        flux_x = ((cx*ua_l + cx*ua_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,1)
        flux_y = ((cy*ua_l + cy*ua_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,2)
        flux_z = ((cz*ua_l + cz*ua_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,3)
        ...
        flux_x = ((cx*ub_l + cx*ub_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,1)
        flux_y = ((cy*ub_l + cy*ub_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,2)
        flux_z = ((cz*ub_l + cz*ub_r)/TWO )  *  mesh%faces(ielem,iface)%norm(:,3)
