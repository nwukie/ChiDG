...
!                sigma_y = eps * fcn
        ...
        !===========================
        ! Unsteady source
        flux =          omega * rho_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux =  flux  + omega * rho_i * ( (sigma_x+sigma_y)/omega )
        


        ! F_r divergence source
        flux = flux   -   (rhobar/y) * v_r * ( (sigma_x + sigma_y)/omega )
        flux = flux   +   (rhobar/y) * v_i * ( ONE - (sigma_x*sigma_y)/(omega*omega) )

        ! F_theta divergence source
        flux = flux   +   (rhobar*real(mod_m,rk)/y) * w_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux   +   (rhobar*real(mod_m,rk)/y) * w_i * ( (sigma_x+sigma_y)/omega )
        ...
        !===========================
        ! Unsteady source
        flux =            omega * u_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux =  flux    + omega * u_i * ( (sigma_x+sigma_y)/omega )
        ...
        !============================
        ! Unsteady source
        flux =              omega * v_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux     +   omega * v_i * ( (sigma_x+sigma_y)/omega )
        

        ! F_r divergence source
!        flux = flux     -   (ONE/(rhobar*y)) * p_r * ( (sigma_x+sigma_y)/omega )
!        flux = flux     +   (ONE/(rhobar*y)) * p_i * ( ONE - (sigma_x*sigma_y)/(omega*omega) )
        ...
        !============================
        ! Unsteady source
        flux =              omega * w_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux     +   omega * w_i * ( (sigma_x+sigma_y)/omega )
        
        ! F_r divergence source

        ! F_theta divergence source
        flux = flux + (real(mod_m,rk)/(rhobar*y)) * p_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux + (real(mod_m,rk)/(rhobar*y)) * p_i * ( (sigma_x+sigma_y)/omega )
        ...
        !============================
        ! Unsteady source
        flux =              omega * p_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux     +   omega * p_i * ( (sigma_x+sigma_y)/omega )


        ! F_r divergence source
        flux = flux   -   (gam*pbar/y)*v_r * ( (sigma_x+sigma_y)/omega )
        flux = flux   +   (gam*pbar/y)*v_i * ( ONE - (sigma_x*sigma_x)/(omega*omega) )

        ! F_theta divergence source
        flux = flux   +   (gam*pbar*real(mod_m,rk)/y)*w_r * ( ONE - sigma_x*sigma_y/(omega*omega) )
        flux = flux   +   (gam*pbar*real(mod_m,rk)/y)*w_i * ( (sigma_x+sigma_y)/omega )
