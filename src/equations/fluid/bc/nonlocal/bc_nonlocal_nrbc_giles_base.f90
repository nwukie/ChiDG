module bc_nonlocal_nrbc_giles_base
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------
    type, public, abstract, extends(bc_state_t) :: nonlocal_nrbc_giles_base_t
    ...
    end type nonlocal_nrbc_giles_base_t
    !*********************************************************************************
...
contains
    ...
    subroutine init(self)
        class(nonlocal_nrbc_giles_base_t),   intent(inout) :: self
    ...
    subroutine initialize_fourier_discretization(self,mesh,group_ID,bc_comm,side)
        ...
        do iradius = 1,size(self%r)
            ...
            do itheta = 1,ntheta
                ...
                user_msg = "bc_nonlocal_nrbc_giles_base%initialize_fourier_discretization: &
...
!        end if
    ...
    function get_face_side(self,worker) result(side)
        class(nonlocal_nrbc_giles_base_t), intent(in)  :: self
        type(chidg_worker_t),   intent(in)  :: worker
...
end module bc_nonlocal_nrbc_giles_base
