module bc_nonlocal_nrbc_lindblad_base_fix
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------
    type, public, abstract, extends(bc_state_t) :: nonlocal_nrbc_lindblad_base_fix_t
    ...
    end type nonlocal_nrbc_lindblad_base_fix_t
    !*********************************************************************************
...
contains
    ...
    subroutine init(self)
        class(nonlocal_nrbc_lindblad_base_fix_t),   intent(inout) :: self
...
!        call self%bcproperties%add('Spatial Periodicity', 'Required')
        ...
        do iradius = 1,nradius
            do itime = 1,ntime
                ...
                if (itime == 1) then
                    ...
                    !shift_sign = ONE
                    negate_dft = .true.    ! this is consistent with Lindblads' formulation
                    shift_sign = -ONE
                else if (itime > 1) then
                    negate_dft = .true.     ! this is consistens with Lindblad's formulation
                    shift_sign = -ONE
                end if
                ...
                do imode = 1,size(density_real_tmp)
                    ...
                    if (side=='A') then
                        ...
                        call chidg_signal(FATAL,"giles_HB_base_fix_t%compute_spatial_dft: Invalid input for argument 'side'. 'A' or 'B'.")
                    end if
    ...
    subroutine interpolate_raux_to_rgq(self,worker,bc_comm,                         &
        ...
        type(point_t),  allocatable, dimension(:)   :: coords
        integer(ik) :: igq, itheta, itime, ierr, ival, lindex, hindex
        real(rk)    :: rinterp, rlow_current, rhigh_current
...
!        end do !igq
        ...
        do igq = 1,size(coords)
            ...
            ! Find interpolation neighbor indices
            rinterp = coords(igq)%c1_

            ! Find next lowest r-value
            lindex          = 1
            rlow_current    = self%r(lindex)
            do ival = 1,size(self%r)
                if ( self%r(ival) < rinterp  .and. self%r(ival) > rlow_current ) then
                    rlow_current = self%r(ival)
                    lindex       = ival
                end if
            end do

            ! Find next highest r-value
            hindex          = size(self%r)
            rhigh_current   = self%r(hindex)
            do ival = 1,size(self%r)
                if ( self%r(ival) > rinterp  .and. self%r(ival) < rhigh_current ) then
                    rhigh_current = self%r(ival)
                    hindex        = ival
                end if
            end do
            ...
            do itheta = 1,size(density_hat_real,2)
                do itime = 1,size(density_hat_real,3)
                    ...
                    density_hat_real_gq(igq,itheta,itime) = density_hat_real(lindex,itheta,itime) + ( density_hat_real(hindex,itheta,itime) - density_hat_real(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
                    density_hat_imag_gq(igq,itheta,itime) = density_hat_imag(lindex,itheta,itime) + ( density_hat_imag(hindex,itheta,itime) - density_hat_imag(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )

                    vel1_hat_real_gq(igq,itheta,itime) = vel1_hat_real(lindex,itheta,itime) + ( vel1_hat_real(hindex,itheta,itime) - vel1_hat_real(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
                    vel1_hat_imag_gq(igq,itheta,itime) = vel1_hat_imag(lindex,itheta,itime) + ( vel1_hat_imag(hindex,itheta,itime) - vel1_hat_imag(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )

                    vel2_hat_real_gq(igq,itheta,itime) = vel2_hat_real(lindex,itheta,itime) + ( vel2_hat_real(hindex,itheta,itime) - vel2_hat_real(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
                    vel2_hat_imag_gq(igq,itheta,itime) = vel2_hat_imag(lindex,itheta,itime) + ( vel2_hat_imag(hindex,itheta,itime) - vel2_hat_imag(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )

                    vel3_hat_real_gq(igq,itheta,itime) = vel3_hat_real(lindex,itheta,itime) + ( vel3_hat_real(hindex,itheta,itime) - vel3_hat_real(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
                    vel3_hat_imag_gq(igq,itheta,itime) = vel3_hat_imag(lindex,itheta,itime) + ( vel3_hat_imag(hindex,itheta,itime) - vel3_hat_imag(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )

                    pressure_hat_real_gq(igq,itheta,itime) = pressure_hat_real(lindex,itheta,itime) + ( pressure_hat_real(hindex,itheta,itime) - pressure_hat_real(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
                    pressure_hat_imag_gq(igq,itheta,itime) = pressure_hat_imag(lindex,itheta,itime) + ( pressure_hat_imag(hindex,itheta,itime) - pressure_hat_imag(lindex,itheta,itime) ) * ( rinterp - self%r(lindex) )/( self%r(hindex) - self%r(lindex) )
...
!                        pressure_imag(iradius,itheta,itime) = HALF*a4_imag(iradius,itheta,itime)  +  HALF*a5_imag(iradius,itheta,itime)
    ...
    subroutine compute_eigenvalues(self, worker, side, lm, omega, vel1_bar, vel2_bar, vel3_bar, c_bar, &
        ...
        if (side == 'A') then
            ...
            call chidg_signal_one(FATAL,"bc_giles_HB_base_fix%compute_eigenvalues: invalid value for input 'side'.",side)
        end if
...
!        end if
    ...
    subroutine initialize_fourier_discretization(self,mesh,group_ID,bc_comm,side)
        ...
        do iradius = 1,size(self%r)
            ...
            do itheta = 1,ntheta
                ...
                user_msg = "bc_nonlocal_nrbc_lindblad_base_fix%initialize_fourier_discretization: &
    ...
    function get_face_side(self,worker) result(side)
        class(nonlocal_nrbc_lindblad_base_fix_t), intent(in)  :: self
        type(chidg_worker_t),   intent(in)  :: worker
...
end module bc_nonlocal_nrbc_lindblad_base_fix
