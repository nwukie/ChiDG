module bc_state_inlet_giles_quasi3d_steady
#include <messenger.h>
    ...
    !>  Steady Quasi-3D Giles Nonreflecting inlet Boundary Condition
    !!
    !!  Options:
    !!      : Average Pressure
    !!      : Pitch
    ...
    !!
    !---------------------------------------------------------------------------------
    type, public, extends(bc_fluid_averaging_t) :: inlet_giles_quasi3d_steady_t
    ...
    end type inlet_giles_quasi3d_steady_t
    !*********************************************************************************
...
contains
    ...
    subroutine init(self)
        class(inlet_giles_quasi3d_steady_t),   intent(inout) :: self

        ! Set name, family
        call self%set_name('Inlet - Giles Quasi3D Steady')
    ...
    subroutine compute_bc_state(self,worker,prop,bc_COMM)
        ...
        do iradius = 1,size(self%r)
            ...
            do imode = 2,nmodes 
                ...
                A3_real = -vel2_bar_r/(ONE + vel3_bar_r)
                A3_imag = -beta/(ONE + vel3_bar_r)

                A4_real = (vel2_bar_r*vel2_bar_r - beta*beta)/((ONE + vel3_bar_r)**TWO)
                A4_imag = TWO*beta/((ONE + vel3_bar_r)**TWO)


                c1_hat_real(imode,iradius) = ZERO
                c1_hat_imag(imode,iradius) = ZERO

                c3_hat_real(imode,iradius) = (A3_real*c5_hat_real(imode,iradius) - A3_imag*c5_hat_imag(imode,iradius))
                c3_hat_imag(imode,iradius) = (A3_imag*c5_hat_real(imode,iradius) + A3_imag*c5_hat_real(imode,iradius))

                c4_hat_real(imode,iradius) = (A4_real*c5_hat_real(imode,iradius) - A4_imag*c5_hat_imag(imode,iradius))
                c4_hat_imag(imode,iradius) = (A4_imag*c5_hat_real(imode,iradius) + A4_imag*c5_hat_real(imode,iradius))
...
!                                           - (A4_imag*c4_hat_real(imode,iradius) + A4_real*c4_hat_imag(imode,iradius))      ! A4*c4 (imag)
        ...
        allocate(c1_hat_real_gq(size(c5_hat_real,1),size(coords)), &
                 c1_hat_imag_gq(size(c5_hat_imag,1),size(coords)), &
                 c3_hat_real_gq(size(c5_hat_real,1),size(coords)), &
                 c3_hat_imag_gq(size(c5_hat_imag,1),size(coords)), &
                 c4_hat_real_gq(size(c5_hat_real,1),size(coords)), &
                 c4_hat_imag_gq(size(c5_hat_imag,1),size(coords)), stat=ierr)
        if (ierr /= 0) call AllocationError
        c1_hat_real_gq = c1_hat_real(1,1)
        c1_hat_imag_gq = c1_hat_real(1,1)
        c3_hat_real_gq = c3_hat_real(1,1)
        c3_hat_imag_gq = c3_hat_real(1,1)
        c4_hat_real_gq = c4_hat_real(1,1)
        c4_hat_imag_gq = c4_hat_real(1,1)
        c1_hat_real_gq = ZERO   
        c1_hat_imag_gq = ZERO
        c3_hat_real_gq = ZERO   
        c3_hat_real_gq = ZERO   
        ...
        expect_zero = [AD_D(1)]
        c1_3d = c1_hat_real_gq(1,:)
        c3_3d = c3_hat_real_gq(1,:)
        c4_3d = c4_hat_real_gq(1,:)
        c1_3d = ZERO
        c3_3d = ZERO
        c4_3d = ZERO
        do igq = 1,size(coords)
        ...
        do patch_ID = 1,mesh%bc_patch_group(group_ID)%npatches()
            do face_ID = 1,mesh%bc_patch_group(group_ID)%patch(patch_ID)%nfaces()
                ...
                if (iface == XI_MIN) then
                    ...
                    call chidg_signal(FATAL,"inlet_giles_quasi3d_steady: analyze_bc_geometry, invalid face indec.")
                end if
    ...
    subroutine initialize_fourier_discretization(self,mesh,group_ID,bc_comm)
        ...
        do iradius = 1,size(self%r)
            ...
            do itheta = 1,ntheta
                ...
                user_msg = "bc_state_inlet_giles_quasi3d_steady%initialize_fourier_discretization: &
...
end module bc_state_inlet_giles_quasi3d_steady
