module bc_state_outlet_nrbc_giles
#include <messenger.h>
    ...
    use bc_nonlocal_nrbc_giles_base,       only: nonlocal_nrbc_giles_base_t
    ...
    !!
    !---------------------------------------------------------------------------------
    type, public, extends(nonlocal_nrbc_giles_base_t) :: outlet_nrbc_giles_t
...
contains
    ...
    subroutine init(self)
        class(outlet_nrbc_giles_t),   intent(inout) :: self

        ! Set name, family
        call self%set_name('Outlet - Nonlocal NRBC Giles')
    ...
    subroutine compute_absorbing_outlet(self,worker,bc_comm,                    &
        ...
        type(AD_D), allocatable, dimension(:)   :: ddensity, dvel1, dvel2, dvel3, dpressure, c1_1d, c2_1d, c3_1d, c4_1d, c5_1d, p_user
...
end module bc_state_outlet_nrbc_giles
