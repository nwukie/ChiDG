module euler_roe_regularized_operator
    ...
    !!
    !------------------------------------------------------------------------------
    type, extends(operator_t), public :: euler_roe_regularized_operator_t

        real(rk)        :: window_size_ctil     = 1.0e-2_rk
        real(rk)        :: minimum_value_ctil   = 1.0e-11_rk
        logical         :: regularize_ctil      = .true.
    ...
    end type euler_roe_regularized_operator_t
    !*******************************************************************************
...
contains
    ...
    subroutine init(self)
        class(euler_roe_regularized_operator_t),   intent(inout)    :: self

        integer             :: unit, msg
        logical             :: file_exists
        real(rk)    :: window_size_ctil, minimum_value_ctil
        logical     :: regularize_ctil

        namelist /regularize_euler_roe_operator/ window_size_ctil, minimum_value_ctil, regularize_ctil

        ! Set operator name
        call self%set_name("Euler Regularized Roe Flux")
        ...
        inquire(file='regularize_euler_roe_operator.nml', exist=file_exists)
        if (file_exists) then
            open(newunit=unit,form='formatted',file='regularize_euler_roe_operator.nml')
            read(unit,nml=regularize_euler_roe_operator,iostat=msg)
            if (msg == 0) self%window_size_ctil = window_size_ctil
            if (msg == 0) self%minimum_value_ctil = minimum_value_ctil
            if (msg == 0) self%regularize_ctil = regularize_ctil
            close(unit)
        end if
    ...
    subroutine compute(self,worker,prop)
        ...
        ctilpre = (gam - ONE)*(Htil - HALF*qtil2)
        if (self%regularize_ctil) ctilpre = ctilpre*sin_ramp(ctilpre, ZERO, self%window_size_ctil) + self%minimum_value_ctil
...
end module euler_roe_regularized_operator
