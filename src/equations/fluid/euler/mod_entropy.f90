        ...
        associate (mesh => data%mesh, sdata => data%sdata, eqnset => data%eqnset, prop => data%eqnset(1)%prop)
            ...
            do idom = 1,data%mesh%ndomains()
                ...
                do ielem = 1,nelem
                    ...
                    !
                    error = sum(entropy_rise * mesh%domain(idom)%elems(ielem)%jinv * mesh%domain(idom)%elems(ielem)%ref_s%weights())


                    !
                    ! Compute element volume
                    !
                    vol = abs(sum(mesh%domain(idom)%elems(ielem)%jinv * mesh%domain(idom)%elems(ielem)%ref_s%weights()))
