...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------
    type, extends(model_t)  :: ideal_gas_rstm_t
...
contains
    ...
    subroutine init(self)   
        class(ideal_gas_rstm_t), intent(inout)   :: self

        call self%set_name('Ideal Gas RSTM')
    ...
    subroutine compute(self,worker)
        class(ideal_gas_rstm_t),     intent(in)      :: self
        type(chidg_worker_t),   intent(inout)   :: worker

        type(AD_D), dimension(:),   allocatable :: &
            density, mom1, mom2, mom3, energy, k, pressure, temperature
        ...
        k  = worker%get_field('Turbulence Kinetic Energy',     'value')
        ...
        pressure = (gam-ONE)*(energy - HALF*( (mom1*mom1) + (mom2*mom2) + (mom3*mom3) )/density - density*k)
...
end module model_ideal_gas_rstm
