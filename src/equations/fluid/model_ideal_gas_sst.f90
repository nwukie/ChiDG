...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------
    type, extends(model_t)  :: ideal_gas_sst_t
...
contains
    ...
    subroutine init(self)   
        class(ideal_gas_sst_t), intent(inout)   :: self

        call self%set_name('Ideal Gas SST')
    ...
    subroutine compute(self,worker)
        class(ideal_gas_sst_t),     intent(in)      :: self
        type(chidg_worker_t),   intent(inout)   :: worker

        type(AD_D), dimension(:),   allocatable :: &
            density, mom1, mom2, mom3, energy, density_k, k, pressure, temperature
        ...
        pressure = (gam-ONE)*(energy - HALF*( (mom1*mom1) + (mom2*mom2) + (mom3*mom3) )/density - density_k)
...
end module model_ideal_gas_sst
