...
#include <messenger.h>
    ...
    !!
    !--------------------------------------------------------------------------------------------
    type, public, extends(equation_builder_t) :: flat_plate_rans
...
contains
    ...
    subroutine init(self)
        class(flat_plate_rans),   intent(inout)  :: self

        call self%set_name('Flat Plate RANS')
    ...
    function build(self,blueprint) result(equation_set)
        class(flat_plate_rans),  intent(in)  :: self
        ...
        !
        call equation_set%set_name('Flat Plate RANS')
...
end module eqn_flat_plate_rans
