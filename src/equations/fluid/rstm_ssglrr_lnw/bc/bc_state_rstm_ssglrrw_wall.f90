...
contains
    ...
    subroutine compute_bc_state(self,worker,prop,bc_COMM)
        ...
        !distance = minval(abs(h))/real(order, rk)
        ...
        do ii = 1, nnodes
            if (ieee_is_nan(density_nutilde_m(ii)%x_ad_)) print *, 'density omega bc is nan, distance ', distance
            if (any(ieee_is_nan(density_nutilde_m(ii)%xp_ad_))) print *, 'density omega bc deriv is nan'
        end do
