module spalart_allmaras_laxfriedrichs_blend
#include <messenger.h>
    ...
    !!
    !-----------------------------------------------------------------------------------------
    type, extends(operator_t), public :: spalart_allmaras_laxfriedrichs_blend_operator_t
    ...
    end type spalart_allmaras_laxfriedrichs_blend_operator_t
    !*****************************************************************************************
...
contains
    ...
    subroutine init(self)
        class(spalart_allmaras_laxfriedrichs_blend_operator_t),   intent(inout)  :: self

        ! Set operator name
        call self%set_name('Spalart-Allmaras LaxFriedrichs Blend Operator')
...
!        integrand = flux_1*norm_1*unorm_1 + flux_2*norm_2*unorm_2 + flux_3*norm_3*unorm_3
        ...
        integrand = integrand + max(abs(diss_m),abs(diss_p))*HALF*diff*norm_1*unorm_1*100._rk
        integrand = integrand + max(abs(diss_m),abs(diss_p))*HALF*diff*norm_2*unorm_2*100._rk
        integrand = integrand + max(abs(diss_m),abs(diss_p))*HALF*diff*norm_3*unorm_3*100._rk
...
end module spalart_allmaras_laxfriedrichs_blend
