...
#include <messenger.h>
    ...
    !>  This functional computes the mass flux balance between an auxiliary and reference surfaces
    !!
    !!  @author Matteo Ugolotti
    !!  @date   5/23/2019
    !!
    !------------------------------------------------------------------------------------------
    type, extends(evaluator_t), public :: mass_flux_balance_t
...
contains
    ...
    subroutine init(self)
        ...
        call self%set_name("Mass Flux Balance")
    ...
    !>  Compute mass flux on auxiliary surface  
    !!  
    !!  @author Matteo Ugolotti
    !!  @date   5/23/2019
    !!
...
!            call chidg_signal(FATAL,'bad coordinate system')
    ...
    !>  Compute mass flux on reference geoemtry 
    !!
    !!  @author Matteo Ugolotti
    !!  @date   5/23/2019
    !!
...
!            call chidg_signal(FATAL,'bad coordinate system')
    ...
    !>  Compute mass flux at reference minus the mass flux at the auxiliary surface
    !!
    !!  @author Matteo Ugolotti
    !!  @date   5/23/2019
    !!
    ...
    function store_value(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_real('mass flux balance')
    ...
    function store_deriv(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_deriv('mass flux balance')
...
end module fcl_mass_flux_balance
