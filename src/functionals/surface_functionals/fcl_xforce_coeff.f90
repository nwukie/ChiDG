...
#include <messenger.h>
    ...
    !>  This functional computes the X-force Coefficient generated by a given surface, using the
    !!  following equation:
    !!
    !!      Cx = Fx/(rho_inf*v_inf^2*0.5) 
    !!
    !!  Two geometries need to be defined:
    !!      - reference -> used to compute the X-force Fx (ex wing, viscous_plate)
    ...
    !!
    !------------------------------------------------------------------------------------------
    type,  extends(evaluator_t), public   :: xforce_coeff_t
...
contains
    ...
    subroutine init(self)
        class(xforce_coeff_t), intent(inout)   :: self

        ! Start defining the evaluator information
        call self%set_name("X-force Coefficient")
    ...
    subroutine check(self)
        ...
        usr_msg_r = "fcl_xforce_coeff: wrong number of reference geometries. The minimum number of reference geometries is 1."
        usr_msg_a = "fcl_xforce_coeff: wrong number of auxiliary geometries. The minimum number of auxiliary geometries is 1."
    ...
    !>  Computing the xforce coefficient 
    ...
    !!
    !!  For xforce coefficient computation, the overall xforce force needs to be divided 
    ...
    subroutine finalize_functional(self,cache) 
        ...
        xforce           = cache%get_value('xforce',          'reference')
        dynamic_pressure = cache%get_value('dynamic pressure','auxiliary')

        call cache%set_value(xforce/dynamic_pressure,'Fx coefficient','reference')
    ...
    function store_value(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_real('Fx coefficient')
    ...
    function store_deriv(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_deriv('Fx coefficient')

    end function store_deriv
    !*********************************************************************************************
    
end module fcl_xforce_coeff
