...
#include <messenger.h>
    ...
    !>  This functional computes the Z-force Coefficient generated by a given surface, using the
    !!  following equation:
    !!
    !!      Cz = Fz/(rho_inf*v_inf^2*0.5) 
    !!
    !!  Two geometries need to be defined:
    !!      - reference -> used to compute the Z-force Fz (ex wing, viscous_plate)
    !!      - auxiliary -> used to compute the area average properties in the free
    !!                     stream (rho_inf & v_inf)
    !!
    !!  NOTE: the Fz coefficient should be divided by the reference area. Since the reference
    ...
    !!
    !------------------------------------------------------------------------------------------
    type,  extends(evaluator_t), public   :: zforce_coeff_t
...
contains
    ...
    subroutine init(self)
        ...
        call self%set_name("Z-force Coefficient")
    ...
    subroutine check(self)
        ...
        usr_msg_r = "fcl_zforce_coeff: wrong number of reference geometries. The minimum number of reference geometries is 1."
        usr_msg_a = "fcl_zforce_coeff: wrong number of auxiliary geometries. The minimum number of auxiliary geometries is 1."
    ...
    !>  Computing the zforce coefficient 
    ...
    subroutine compute_functional(self,worker,cache)
        ...
        type(AD_D), allocatable, dimension(:) :: pressure, tau_11, tau_22, tau_33,  &
                                                 ...
                                                 tau_31, tau_32, zforce_gq, stress_x, &
                                                 stress_y, stress_z,                &
        ...
        ! Compute zforce Force at quadrature nodes
        zforce_gq = stress_z
    ...
    !!
    !!  For zforce coefficient computation, the overall zforce force needs to be divided 
    ...
    function store_value(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_real('Fz coefficient')
    ...
    function store_deriv(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_deriv('Fz coefficient')

    end function store_deriv
    !*********************************************************************************************
    
end module fcl_zforce_coeff
