module fcl_test_1D_face_integral
#include <messenger.h>
    ...
    !!
    !------------------------------------------------------------------------------------------
    type,  extends(evaluator_t), public   :: test_1D_face_integral_t
    ...
    end type test_1D_face_integral_t
    !******************************************************************************************
...
contains
    ...
    subroutine init(self)
        class(test_1D_face_integral_t), intent(inout)   :: self

        call self%set_name("Test 1D Face Integral")
    ...
    subroutine check(self)
        ...
        usr_msg_r = "fcl_test_1D_face_integral: wrong number of reference geometries. The minimum number of reference geometries required is 1"
        usr_msg_a = "fcl_test_1D_face_integral: This functional does not require any auxiliary geometry. Please, use `chidg edit` to remove the input auxiliary geometry"
    ...
    !>  Compute Kinetic integral Integral of a single element.
    !!
    !!  @author Matteo Ugolotti
    !!  @date   12/17/2017
    !!
    ...
    subroutine compute_functional(self,worker,cache)
        ...
        ! Get primary fields
        u = worker%get_field('u','value','face interior')
    ...
    function store_value(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_real('u integral')
    ...
    function store_deriv(self,cache) result(res) 
        ...
        res = cache%ref_cache%get_deriv('u integral')
...
end module fcl_test_1D_face_integral
