...
#include <messenger.h>
    ...
    !>  Wendland's C0 compactly supported RBF 
    !!
    !!  @author Eric Wolf
    !!  @date  8/4/2017 
    !!
    !!
    !-------------------------------------------------------------------------------
    type, extends(radial_basis_function_t), public :: wc0_rbf
...
contains
    ...
    subroutine init(self)
        class(wc0_rbf),  intent(inout)  :: self

        !
        ! Set function name
        !
        call self%set_name("wc0_rbf")
    ...
    function compute_grad(self,eval_node,support_node,support_radius) result(val)
        ...
        if (radius_norm < ONE) then
            ...
            val = -TWO*(ONE-radius_norm)*radius_norm_grad
        else
            val = ZERO
        end if
