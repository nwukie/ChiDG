...
contains
    ...
    subroutine read_grid_hdf(filename, data)
        ...
        associate (domains => data%domains)
    ...
    !                allocate(zpts, mold=xpts)   ! bug in gcc
                    ...
                    call domains(idom)%init_geom(idom,nterms_c,points)
...
!        real(rk), allocatable           :: var(:,:)
        ...
        associate ( domains => data%domains, sdata => data%sdata )
            ...
            do igrp = 0,nmembers-1
                ...
                if (gname(1:2) == 'D_') then
                    ...
                    ElementsEqual = (size(sdata%q%dom(idom)%lvecs) == size(var,2))
                    if (ElementsEqual) then
                        !  Loop through elements and assign 'variable' values
                        do ielem = 1,domains(idom)%mesh%nelem
                            call sdata%q%dom(idom)%lvecs(ielem)%setvar(ivar,var(:,ielem))
                        end do
                    else
            ...
            do igrp = 0,nmembers-1
                ...
                if (gname(1:2) == 'D_') then
                    ...
                    do ielem = 1,domains(idom)%mesh%nelem
                            ...
                            var(:,ielem) = sdata%q%dom(idom)%lvecs(ielem)%getvar(ivar)
                    end do
