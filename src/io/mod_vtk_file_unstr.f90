...
contains
    ...
    subroutine close_vtk_unstr(file_name)
        ...
        !
        ! Open existing vtk file (show an error if file doesn't exist) and write out the 
        ! closing statements common to all xml vtk files
        !
        inquire(file = trim(file_name), exist=exist)
        if(exist) then
    ...
    subroutine init_piece_unstr(file_name,num_pts,num_cells)
        ...
        !
        ! Open existing vtk file (show error if file doesn't exist) and initialize a new 
        ! unstructured grid piece 
    ...
    subroutine write_piece_coord(file_name,num_pts,X_coord,Y_coord,Z_coord)
        ...
        ! Initialize the points field (<Points>) for the current piece and write out x,y,z 
        ! coordinate values.
    ...
    subroutine write_piece_data(file_name,noeq,num_pts,cons_var,cons_var_val)
        ...
        !
        ! Open existing vtk file (show error if file doesn't exist)
        ! Initialize point data field (<PointData...>) for the current piece, currently only 
