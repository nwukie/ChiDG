...
#include <messenger.h>
    ...
    !!
    !---------------------------------------------------------------------------------------------
    type, public, extends(linear_solver_t) :: fgmres_cgs_mpc_t
        integer(ik) :: m = 2000
    contains
        procedure   :: solve
    end type fgmres_cgs_mpc_t
    !*********************************************************************************************
...
contains
    ...
    subroutine solve(self,A,x,b,M,solver_controller)
        class(fgmres_cgs_mpc_t),    intent(inout)               :: self
...
!        type(precon_HB_t)   :: M_HB
!        type(precon_ILU0_t) :: M_ILU0
!
!
!        !
!        ! Reset/Start timers
...
!            if (solver_controller%update_preconditioner(A)) call M_A%update(A,b)
!            if (solver_controller%update_preconditioner(A)) call M_B%update(A,b)
!        else
!            call M_A%update(A,b)
!            call M_B%update(A,b)
!        end if
...
end module type_fgmres_cgs_mpc
