...
#include <messenger.h>
    ...
    !>  Flow In Cylinder - Case1 prescribed motion. 
    !!
    !!  @author Nathan A. Wukie
    !!  @date   10/02/2019 
    !!
    !-------------------------------------------------------------------------------
    type, extends(prescribed_mesh_motion_function_t), public :: fic_case1_pmmf
        private
...
contains
    ...
    subroutine init(self)
        class(fic_case1_pmmf),  intent(inout)  :: self

        ! Set function name
        call self%set_name("Flow In Cylinder - Case1")
    ...
    function compute_pos(self,time,node) result(val)
        ...
        real(rk)    :: alpha, x0, y0, x_ale, y_ale, &
                       xc, yc, height, theta

        alpha = time**TWO*(THREE-time)/FOUR

        if (time <= TWO) height = alpha
    ...
    function compute_vel(self,time,node) result(val)
        ...
        integer(ik) :: ivar
        real(rk)    :: val(3)
        real(rk)    :: alpha, dalphadt, x0, y0, vx_ale, vy_ale, &
