module type_prescribed_mesh_motion_function
#include <messenger.h>
    ...
    !>  Prescribed mesh motion class. Adapted from function class.
    !!
    !!  Provides for the computation of grid position and velocity.
    !!
    !!
    !!  @author Eric Wolf
    !!  @date  3/15/2017
    ...
    !!
    !--------------------------------------------------------------------------------------------
    type, public, abstract :: prescribed_mesh_motion_function_t
    ...
    contains
        ...
        ! Function copute
        procedure(compute_pos_interface),   deferred    :: compute_pos          !< Elemental function definition
        procedure(compute_vel_interface),   deferred    :: compute_vel         !< Elemental function definition

    end type prescribed_mesh_motion_function_t
    !********************************************************************************************
...
contains
    ...
    function get_name(self) result(fname)
        class(prescribed_mesh_motion_function_t),  intent(in)  :: self
...
end module type_prescribed_mesh_motion_function
