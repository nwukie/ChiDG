...
#include <messenger.h>
    ...
    !!
    !------------------------------------------------------------------------------------------
    type, extends(nonlinear_solver_t), public :: quasi_newton_sst_t
...
contains
    ...
    subroutine solve(self,data,system,linear_solver,preconditioner,solver_controller)
        ...
        associate ( q   => data%sdata%q,    &
            ...
            do while ( absolute_convergence .and. relative_convergence )
                ...
                ! Unless the solution is sufficiently converged, zero out the off-diagonal blocks of the Jacobian
                ! that couple the mean flow/turbulence model equations
                if (1.0e-2_rk < resid) call modify_jacobian_sst(data)
                ...
                if ( ieee_is_nan(resid) ) then
                    call chidg_signal(FATAL,"quasi_newton_sst%solve: NaN residual calculation. Check initial solution and operator objects.")
                end if
    ...
    subroutine destructor(self)
        type(quasi_newton_sst_t),      intent(in) :: self
...
end module type_quasi_newton_sst
