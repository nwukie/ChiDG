...
#include <messenger.h>
    ...
    use type_RASILU0_OVERSET_send,  only: RASILU0_OVERSET_send_t
    use type_RASILU0_OVERSET_recv,  only: RASILU0_OVERSET_recv_t
    ...
    !!
    !------------------------------------------------------------------------------------------
    type, extends(preconditioner_t) :: precon_RASILU0_OVERSET_t

        type(chidg_matrix_t)            :: LD       !< Lower-Diagonal matrix for local problems

        type(RASILU0_OVERSET_send_t)    :: send     !< Overlapping data to send to other processors
        type(RASILU0_OVERSET_recv_t)    :: recv     !< Overlapping data to receive from other processors
    ...
    contains
        ...
        procedure   :: comm_send => RASILU0_comm_send
        procedure   :: comm_recv => RASILU0_comm_recv
        procedure   :: comm_wait => RASILU0_comm_wait

    end type precon_RASILU0_OVERSET_t
    !******************************************************************************************
...
contains
    ...
    subroutine init(self,data)
        class(precon_RASILU0_OVERSET_t),    intent(inout)   :: self
    ...
    subroutine update(self,A,b)
        ...
        do idom = 1,ndom
            ...
            !
            do ielem = 1,size(A%dom(idom)%lblks,itime)
                ...
                do ielem = 1,size(self%recv%dom(idom)%comm(icomm)%elem)
                    do iblk = 1,self%recv%dom(idom)%comm(icomm)%elem(ielem)%lower%size()
                        ...
                        if ( parent_proc /= IRANK ) then
                            ...
                            iblk_diag_parent = self%LD%dom(idom)%lblks(eparent_l,itime)%get_diagonal()
                            ...
                            associate( lower      => self%recv%dom(idom)%comm(icomm)%elem(ielem)%blks(ilower)%mat,  &
                                       upper_diag => self%LD%dom(idom)%lblks(eparent_l,itime)%data_(iblk_diag_parent)%mat )
                                lower = matmul(lower,upper_diag)
                            end associate
                        end if
                        ...
                        if ( parent_proc /= IRANK ) then
                            ...
                            associate ( diag  => self%recv%dom(idom)%comm(icomm)%elem(ielem)%blks(iblk_diag)%mat,   &
                                        lower => self%recv%dom(idom)%comm(icomm)%elem(ielem)%blks(ilower)%mat,      &
                                        trans => A%dom(idom)%lblks(eparent_l,itime)%data_(trans_blk)%mat )
                                diag = diag - matmul(lower,trans)
                            end associate
                        end if
    ...
    function apply(self,A,v,z_old) result(z)
        ...
        integer(ik)         :: ielem, dparent_l, eparent_l, irow, icol, idom, ndom, ilower, iupper, &
                               ...
                               eparent_g_lower, ilowerA, ilowerLD, itime, matrix_proc, vector_proc
        logical             :: interior_block, local_multiply
            ...
            do irow = 1,size(self%LD%dom(idom)%lblks,itime)
                ...
                do icol = 1,A%dom(idom)%local_lower_blocks(irow,itime)%size()
                    ...
                    if ( A%dom(idom)%lblks(irow,itime)%parent_proc(ilowerA) == IRANK) then
                        ...
                        associate ( zvec     => z%dom(idom)%vecs(irow)%vec, &
                                    coupling => self%LD%dom(idom)%lblks(irow,itime)%data_(ilowerLD)%mat, &
                                    neighbor => z%dom(idom)%vecs(eparent_l)%vec )
                            zvec = zvec - matmul(coupling,neighbor)
                        end associate
            ...
            do icomm = 1,size(self%recv%dom(idom)%comm)
                ...
                do ielem = 1,size(self%recv%dom(idom)%comm(icomm)%elem)
                    ...
                    do iblk = 1,self%recv%dom(idom)%comm(icomm)%elem(ielem)%lower%size()
                        ...
                        if ( parent_proc == IRANK ) then
                            associate ( zvec     => z%recv%comm(recv_comm_diag)%dom(recv_domain_diag)%vecs(recv_element_diag)%vec, &
                            ...
                            associate ( zvec     => z%recv%comm(recv_comm_diag)%dom(recv_domain_diag)%vecs(recv_element_diag)%vec, &
                                        coupling => self%recv%dom(idom)%comm(icomm)%elem(ielem)%blks(ilower)%mat, &
                                        neighbor => z%recv%comm(recv_comm)%dom(recv_domain)%vecs(recv_element)%vec )
                                zvec = zvec - matmul(coupling,neighbor)
                            end associate
            ...
            do icomm = size(self%recv%dom(idom)%comm),1,-1
                do ielem = size(self%recv%dom(idom)%comm(icomm)%elem),1,-1
                    ...
                    associate (zvec => z%recv%comm(recv_comm_diag)%dom(recv_domain_diag)%vecs(recv_element_diag)%vec, &
                                ...
                                neighbor => z%recv%comm(recv_comm_diag)%dom(recv_domain_diag)%vecs(recv_element_diag)%vec )
                        zvec = matmul(coupling,neighbor)
                    end associate
...
!!!
            ...
            do irow = size(A%dom(idom)%lblks,1),1,-1
                ...
                do icol = 1,A%dom(idom)%local_upper_blocks(irow,itime)%size()
                    ...
                    if (interior_block) then
                        eparent_l = A%dom(idom)%lblks(irow,1)%eparent_l(iupper)
                        overset%dom(idom)%vecs(irow)%vec = overset%dom(idom)%vecs(irow)%vec - matmul(A%dom(idom)%lblks(irow,1)%data_(iupper)%mat, overset%dom(idom)%vecs(eparent_l)%vec)
                    else
                        recv_comm    = A%dom(idom)%lblks(irow,1)%data_(iupper)%recv_comm
                        recv_domain  = A%dom(idom)%lblks(irow,1)%data_(iupper)%recv_domain
                        recv_element = A%dom(idom)%lblks(irow,1)%data_(iupper)%recv_element
                        overset%dom(idom)%vecs(irow)%vec = overset%dom(idom)%vecs(irow)%vec - matmul(A%dom(idom)%lblks(irow,1)%data_(iupper)%mat, overset%recv%comm(recv_comm)%dom(recv_domain)%vecs(recv_element)%vec)
                    end if
                ...
                ! Diagonal block
                diag_irow = self%LD%dom(idom)%lblks(irow,1)%get_diagonal()
                overset%dom(idom)%vecs(irow)%vec = matmul(self%LD%dom(idom)%lblks(irow,1)%data_(diag_irow)%mat, overset%dom(idom)%vecs(irow)%vec)
            ...
            do idom_send = 1,size(self%send%comm(icomm)%dom)
                ...
                do ielem_send = 1,self%send%comm(icomm)%dom(idom_send)%elem_send%size()
                    ...
                    do iblk_send = 1,size(send_blocks)
                        ...
                        nrows = size(A%dom(idom)%lblks(ielem,itime)%data_(iblk)%mat,1)
                        ncols = size(A%dom(idom)%lblks(ielem,itime)%data_(iblk)%mat,2)
                        call MPI_ISend(A%dom(idom)%lblks(ielem,itime)%data_(iblk)%mat, nrows*ncols, MPI_REAL8, proc, idomain_g, ChiDG_COMM, request, ierr)
    ...
    end subroutine RASILU0_comm_send
    !******************************************************************************************
    ...
    !!
    !------------------------------------------------------------------------------------------
    subroutine RASILU0_comm_recv(self)
        class(precon_RASILU0_OVERSET_t),   intent(inout)   :: self
    ...
    end subroutine RASILU0_comm_recv
    !******************************************************************************************
    ...
    !!
    !------------------------------------------------------------------------------------------
    subroutine RASILU0_comm_wait(self)
        class(precon_RASILU0_OVERSET_t),    intent(inout)   :: self
    ...
    end subroutine RASILU0_comm_wait
    !******************************************************************************************



end module precon_RASILU0_OVERSET
