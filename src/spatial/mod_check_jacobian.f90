...
contains
    ...
    subroutine check_jacobian_boundary_average_flux(domain,ielem,iface,ivar,iblk,blk_dnad,blk_fd)
        type(domain_t),      intent(inout)       :: domain
        integer(ik),         intent(in)          :: ielem, iface, ivar, iblk
        type(densematrix_t), intent(inout)       :: blk_dnad, blk_fd
   ...
   subroutine check_jacobian_boundary_upwind_flux(domain,ielem,iface,ivar,iblk,blk_dnad,blk_fd)
        type(domain_t),      intent(inout)       :: domain
        integer(ik),         intent(in)          :: ielem, iface, ivar, iblk
        type(densematrix_t), intent(inout)       :: blk_dnad, blk_fd
