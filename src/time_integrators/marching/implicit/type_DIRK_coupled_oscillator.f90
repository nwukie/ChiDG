...
!!  time integrator, DIRK_coupled_oscillator.
!!
!!  Object definitions:
!!  -------------------
!!      1: DIRK_coupled_oscillator_t                       the new time_integrator_t itself. Defines how to take a
!!                                      DIRK_coupled_oscillator step.
!!      2: assemble_DIRK_coupled_oscillator_t              a system_assembler_t object that implements how to assemble
...
!!                                      the system itself.
!!      3: DIRK_coupled_oscillator_solver_controller_t     a solver_controller_t object that implements rules
...
!!
!------------------------------------------------------------------------------------
module type_DIRK_coupled_oscillator
#include<messenger.h>
    ...
    !!
    !--------------------------------------------------------------------------------
    type, extends(time_integrator_marching_t),  public      :: DIRK_coupled_oscillator_t
...
contains
    ...
    !>  Initialize the DIRK_coupled_oscillator_t time integrator
    !!
    ...
    !! 
    !!  where b_{i}, A_{ij} are the coefficients of the DIRK_coupled_oscillator method. The Newton linearization 
   ...
   !***************************************************************************************
    ...
    !>  Assemble the system for the DIRK_coupled_oscillator equations with temporal contributons
    !!
    !!  @author Mayank Sharma
    !!  @date   5/20/2017
    !!
    ...
    function update_lhs(self,A,niter,residual_ratio) result(update)
        class(DIRK_coupled_oscillator_solver_controller_t),    intent(inout)   :: self
...
end module type_DIRK_coupled_oscillator
