...
contains
    ...
    !!
    !--------------------------------------------------------------------
    subroutine compute_euler_eigenmodes_new_new()
        ...
        real(rk),         allocatable,   dimension(:,:) :: mass, rmass, stiff, bound1, bound2, zero_matrix, eigenvector_mag, val, dat, stencil, identity, ridentity, bc, smooth
...
!        end do
        ...
        !
        ! Construct Second-order stencil matrix
        !
        stencil = 0._rk
        stencil(1,  1:3) = [-3._rk, 4._rk, -1._rk]
        stencil(res,res-2:res) = [1._rk, -4._rk,  3._rk]
        do i = 2,res-1
            stencil(i,i-1) = -1._rk
            stencil(i,i+1) =  1._rk
        end do
        ...
        ! Scale by dr
        stencil = (1._rk/(2._rk*dr))*stencil
        ...
        !
        ! Construct fourth-order smoothing operator and add to stencil
        !
        smooth = 0._rk
        smooth(1,  1:6) = [3._rk, -14._rk, 26._rk, -24._rk, 11._rk, -2._rk]
        !smooth(2,  1:5) = [-3._rk, -10._rk,  18._rk, -6._rk,  1._rk]
        !smooth(res-1,res-4:res) = [-1._rk,  6._rk, -18._rk,  10._rk,  3._rk]
        smooth(res,  res-5:res) = [-2._rk, 11._rk, -24._rk, 26._rk, -14._rk, 3._rk]
        do i = 3,res-2
            smooth(i,i-2) =  1._rk
            smooth(i,i-1) = -4._rk
            smooth(i,i)   =  6._rk
        ...
        ! Scale by dr
        smooth = (1._rk/(dr*dr*dr*dr))*smooth



        !stencil = stencil + 0.01*dr*dr*dr*smooth
...
!        end do
        ...
        irow_start = 1 + res*(irow-1)
        icol_start = 1 + res*(icol-1)
        M2(irow_start:irow_start + (res-1),icol_start:icol_start + (res-1)) = M2(irow_start:irow_start+(res-1),icol_start:icol_start+(res-1)) + cmplx(zero_matrix,-identity*omega)
        ...
        !bc(:,1)   = ZERO
        !bc(:,res) = ZERO
        M2(irow_start:irow_start + (res-1),icol_start:icol_start + (res-1)) = M2(irow_start:irow_start+(res-1),icol_start:icol_start+(res-1)) + cmplx(zero_matrix,-bc*omega)
...
!        end do
!
!        print*, 'A reduced(real): '
!        do i = 1,size(A_rr,1)
!            print*, realpart(A_rr(i,:))
!        end do
!
!
!        print*, 'A(real): '
...
!        end do
!
!        print*, 'A reduced(imag): '
!        do i = 1,size(A_rr,1)
!            print*, imagpart(A_rr(i,:))
!        end do
...
!        end do
    ...
    end subroutine compute_euler_eigenmodes_new_new
    !********************************************************************
    ...
    !!
    !--------------------------------------------------------------------
    subroutine compute_euler_eigenmodes_new()
...
!        end do
    ...
    end subroutine compute_euler_eigenmodes_new
    !********************************************************************
    ...
    !!
    !--------------------------------------------------------------------
    subroutine compute_euler_eigenmodes(grid_file)
